...
$|=1;
my ($infile1,$infile2,$min_length,$is_fasta,$outfile_combined,$comb,$sorted,$tosort,@infiles);
my $maxmemory = 20971520; # kb. 20gb
GetOptions(
          'i:s{2}' =>\@infiles,
...
unless ($infile1 && $infile2 && -s $infile1 && -s $infile2 && $infile1 ne $infile2){
...
my ($counter,%hash);
if ($sorted){
	### cat $1 |awk '{OFS="\t"; getline seqs; getline sep; getline quals; print 0$0,seqs,sep,quals}'|sort -k1,1|awk '{OFS="\n";sub(/^0/,"");print $1,$2,"+",$4}'
	...
	system("cat $infile1 ".'|awk \'{OFS="\t"; getline seqs; getline sep; getline quals; print 0$0,seqs,sep,quals}\'|sort -S'.$buffer
	.' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2,"+",$4}\'  >'." $infile1.sorted") if !-s "$infile1.sorted" && !$is_fasta;
	system("cat $infile1 ".'|awk \'{OFS="\t"; getline seqs; print 0$0,seqs}\'|sort -S'.$buffer
	.' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2}\'  >'." $infile1.sorted") if !-s "$infile1.sorted" && $is_fasta;
	sleep(3);
	print "File $infile2\n";
	$buffer = int(((-s $infile2) + ((-s $infile2) * 0.5))/1024);
	$buffer = ($buffer < $maxmemory) ? $buffer : $maxmemory;
	system("cat $infile2 ".'|awk \'{OFS="\t"; getline seqs; getline sep; getline quals; print 0$0,seqs,sep,quals}\'|sort -S'.$buffer
	.' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2,"+",$4}\'  >'." $infile2.sorted") if !-s "$infile2.sorted" && !$is_fasta;
	system("cat $infile2 ".'|awk \'{OFS="\t"; getline seqs; print 0$0,seqs}\'|sort -S'.$buffer
	.' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2}\'  >'." $infile2.sorted") if !-s "$infile2.sorted" && $is_fasta;
	$infile1=$infile1.".sorted";
	$infile2=$infile2.".sorted";
	&process_sorted($infile1,$infile2);
	unlink($infile1);
	unlink($infile2);
}elsif($tosort && $infile1=~/.sorted$/ && $infile2=~/.sorted$/){
...
sub process_sorted($$){
	...
	open (OUTS1,">$large_file.mated.singletons.fastq");
	open (OUTS2,">$small_file.mated.singletons.fastq");
	open (OUT1,">$large_file.mated.fastq");
	open (OUT2,">$small_file.mated.fastq");
	open (IN1,$large_file) ||die;
	open (IN2,$small_file) ||die;
	while (my $sid1=<IN1>){
		chomp($sid1);
		$sid1=~s/^[>@](\S+)\s*.*$/$1/;
		...
		if ($is_fasta){
			&decide_sort_print($sid1,$seq1,$sid2,$seq2);
			$counter+=length($sid1)+length($seq1);
		}else{
			&decide_sort_print($sid1,$seq1,$qid1,$qlt1,$sid2,$seq2,$qid2,$qlt2);
			$counter+=length($sid1)+length($seq1)+length($qid1)+length($qlt1);
		}
		if ( $counter =~ /00000$/ ) {print $timer->report( "eta: %E min, %40b %p\r", $counter );}
	}
	while (my $sid2=<IN2>){
		my $seq2=<IN2>;
		my ($qid2,$qlt2);
...
sub decide_sort_print(){
	...
	if (!$sid1 && $sid2){
		print OUTS2 $head_sign.$sid2."2\n".$seq2."+\n".$qlt2 if !$is_fasta;
		print OUTS2 $head_sign.$sid2."2\n".$seq2 if $is_fasta;
		return;
	}elsif(!$sid2 && $sid1){
		print OUTS1 $head_sign.$sid1."1\n".$seq1."+\n".$qlt1 if !$is_fasta;
		print OUTS1 $head_sign.$sid1."1\n".$seq1 if $is_fasta;
		return;
	}elsif(!$sid1 && !$sid2){
		return;
	...
	if ($sid1 eq $sid2){
		...
		print OUT1 $head_sign.$sid1."1\n".$seq1."+\n".$qlt1 if !$is_fasta;
		print OUT1 $head_sign.$sid1."1\n".$seq1 if $is_fasta;
		print OUT2 $head_sign.$sid2."2\n".$seq2."+\n".$qlt2 if !$is_fasta;
		print OUT2 $head_sign.$sid2."2\n".$seq2 if $is_fasta;
		if ($comb){
			print OUT $head_sign.$sid1."1\n".$seq1."+\n".$qlt1 if !$is_fasta;
			print OUT $head_sign.$sid1."1\n".$seq1 if $is_fasta;
			print OUT $head_sign.$sid2."2\n".$seq2."+\n".$qlt2 if !$is_fasta;
			print OUT $head_sign.$sid2."2\n".$seq2 if $is_fasta;
		}
	}elsif($sid1 ge $sid2){
		print OUTS2 $head_sign.$sid2."2\n".$seq2."+\n".$qlt2 if !$is_fasta;
		...
		chomp($sid2);
      		...
      		$sid2=~s/\d+$//;
                ...
                $qid2=<IN2> if !$is_fasta;
                $qlt2=<IN2> if !$is_fasta;
		&decide_sort_print($sid1,$seq1,$sid2,$seq2) if $is_fasta;
		&decide_sort_print($sid1,$seq1,$qid1,$qlt1,$sid2,$seq2,$qid2,$qlt2) if !$is_fasta;
	}else{
		print OUTS1  $head_sign.$sid1."1\n".$seq1."+\n".$qlt1 if !$is_fasta;
		print OUTS1  $head_sign.$sid1."1\n".$seq1 if $is_fasta;
		$sid1=<IN1>;
...
sub print_hashed_data($){
	...
	while (my $seqdesc=<IN1>){
		...
		if ($hash{$id}{'data'}){
			# too small
			if ($min_length && ($hash{$id}{'length'} < $min_length || length($seq) < $min_length)){
	...
	close IN1;close OUT1;close OUT2;close OUT1B;close OUT1C;close OUT2C;
	close OUT if $comb;
}
